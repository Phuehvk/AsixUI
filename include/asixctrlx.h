...
#ifndef _ASIXCTRLX_INC_
#define _ASIXCTRLX_INC_
...
#error asixctrlx.h requires asixapp.h to be included first
#endif

#ifndef _ASIXCTRLS_INC_
#error asixctrlx.h requires asixctrls.h to be included first
#endif
...
namespace Asix
{
	...
	public:
		...
		CBitmapButtonImpl(DWORD dwExtendedStyle = BMPBTN_AUTOSIZE, HIMAGELIST hImageList = NULL) : m_ImageList(hImageList), m_dwExtendedStyle(dwExtendedStyle), m_lpstrToolTipText(NULL), m_fMouseOver(0), m_fFocus(0), m_fPressed(0)
		{
	...
	{
	public:
		DECLARE_WND_SUPERCLASS(_T("AsixUI_BitmapButton"), GetWndClassName())

		CBitmapButton(DWORD dwExtendedStyle = BMPBTN_AUTOSIZE, HIMAGELIST hImageList = NULL) : CBitmapButtonImpl<CBitmapButton>(dwExtendedStyle, hImageList)
		{ }
	};
	...
	{
	public:
		DECLARE_WND_SUPERCLASS(_T("AsixUI_CheckListView"), GetWndClassName())
	};
	...
	public:
		...
		// Constructor/Destructor
		CHyperLinkImpl(DWORD dwExtendedStyle = HLINK_UNDERLINED) : m_lpstrLabel(NULL), m_lpstrHyperLink(NULL),	m_hCursor(NULL), m_hFontLink(NULL), m_hFontNormal(NULL), m_clrLink(RGB(0, 0, 255)), m_clrVisited(RGB(128, 0, 128)), m_dwExtendedStyle(dwExtendedStyle), m_bPaintLabel(true), m_bVisited(false), m_bHover(false), m_bInternalLinkFont(false), m_bInternalNormalFont(false)
		{
		...
		void Init()
		{
			...
			if(::GetClassName(m_hWnd, szBuffer, cchBuff))
			{
				if(lstrcmpi(szBuffer, _T("static")) == 0)
				{
					...
					if(dwStyle == SS_ICON || dwStyle == SS_BLACKRECT || dwStyle == SS_GRAYRECT || dwStyle == SS_WHITERECT || dwStyle == SS_BLACKFRAME || dwStyle == SS_GRAYFRAME || dwStyle == SS_WHITEFRAME || dwStyle == SS_OWNERDRAW || dwStyle == SS_BITMAP || dwStyle == SS_ENHMETAFILE)
						m_bPaintLabel = false;
				}
			}
	...
	{
	public:
		DECLARE_WND_CLASS(_T("AsixUI_HyperLink"))
	};
	...
	public:
		// Constructor/destructor
		CCustomWaitCursor(ATL::_U_STRINGorID cursor, bool bSet = true, HINSTANCE hInstance = NULL) : CWaitCursor(false, IDC_WAIT, true)
		{
			if(hInstance == NULL)
	...
	public:
		...
		BOOL SetPanes(int* pPanes, int nPanes, bool bSetText = true)
		{
			...
			CTempBuffer<int, _ASIX_STACK_ALLOC_THRESHOLD> buff;
	...
	{
	public:
		DECLARE_WND_SUPERCLASS(_T("AsixUI_MultiPaneStatusBar"), GetWndClassName())
	};
	...
	{
	public:
		DECLARE_WND_CLASS_EX(_T("AsixUI_PaneContainer"), 0, -1)
	};
	...
	public:
		...
		CSortListViewImpl() : m_bSortDescending(false), m_bCommCtrl6(false), m_iSortColumn(-1), m_fmtOldSortCol(0), m_hbmOldSortCol(NULL), m_dwSortLVExtendedStyle(SORTLV_USESHELLBITMAPS), m_bUseWaitCursor(true)
		{
			DWORD dwMajor = 0;
			DWORD dwMinor = 0;
	...
	{
	public:
		DECLARE_WND_SUPERCLASS(_T("AsixUI_SortListViewCtrl"), GetWndClassName())
	};
	...
	public:
		...
		bool SetPageTitle(int nPage, LPCTSTR lpstrTitle)
		{
			...
			CTempBuffer<TCHAR, _ASIX_STACK_ALLOC_THRESHOLD> buff;
		...
		LRESULT OnTabMouseMove(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM lParam, BOOL& bHandled)
		{
			...
			if(m_bTabCapture)
			{
				...
				if(!m_bTabDrag)
				{
					if(abs(m_ptStartDrag.x - GET_X_LPARAM(lParam)) >= ::GetSystemMetrics(SM_CXDRAG) || abs(m_ptStartDrag.y - GET_Y_LPARAM(lParam)) >= ::GetSystemMetrics(SM_CYDRAG))
					{
						T* pT = static_cast<T*>(this);
	...
	{
	public:
		DECLARE_WND_CLASS_EX(_T("AsixUI_TabView"), 0, COLOR_APPWORKSPACE)
	};

}	// namespace Asix

#endif // _ASIXCTRLX_INC_