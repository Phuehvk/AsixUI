...
#error asixmisc.h requires asixapp.h to be included first
#endif


#ifdef _ATL_TMP_NO_CSTRING
#define _ASIX_NO_CSTRING
#endif

#if defined(_ASIX_USE_CSTRING) && defined(_ASIX_NO_CSTRING)
#error Conflicting options - both _ASIX_USE_CSTRING and _ASIX_NO_CSTRING are defined
#endif // defined(_ASIX_USE_CSTRING) && defined(_ASIX_NO_CSTRING)

#if !defined(_ASIX_USE_CSTRING) && !defined(_ASIX_NO_CSTRING)
#define _ASIX_USE_CSTRING
#endif // !defined(_ASIX_USE_CSTRING) && !defined(_ASIX_NO_CSTRING)

#ifndef _ASIX_NO_CSTRING
...
namespace Asix
{
	...
	#endif // !_ASIX_NO_WTYPES


	// Asix::CSize or ATL::CSize scalar operators 

	#if !defined(_ASIX_NO_SIZE_SCALAR) && (!defined(_ASIX_NO_WTYPES) || defined(__ATLTYPES_H__))
	...
	#endif // !defined(_ASIX_NO_SIZE_SCALAR) && (!defined(_ASIX_NO_WTYPES) || defined(__ATLTYPES_H__))
		...
		BOOL FormatV(LPCTSTR lpszFormat, va_list argList)
		{
			...
			for (LPCTSTR lpsz = lpszFormat; *lpsz != _T('\0'); lpsz = ::CharNext(lpsz))
			{
				...
				else
				{
					...
					case _T('g'):
					case _T('G'):
						ATLASSERT(!"Floating point (%%e, %%E, %%f, %%g, and %%G) is not supported by the Asix::CString class.");
						#ifndef _DEBUG
						::OutputDebugString(_T("Floating point (%%e, %%f, %%g, and %%G) is not supported by the Asix::CString class."));
						::DebugBreak();
						#endif // !_DEBUG
						break;
	...
	#ifndef _ASIX_MRUEMPTY_TEXT
	#define _ASIX_MRUEMPTY_TEXT	_T("(empty)")
	#endif
	...
	inline bool AtlCompactPath(LPTSTR lpstrOut, LPCTSTR lpstrIn, int cchLen)
	{
		...
		{
			if((pPath[0] == _T('\\') || pPath[0] == _T(':') || pPath[0] == _T('/'))	&& pPath[1] && pPath[1] != _T('\\') && pPath[1] != _T('/'))
