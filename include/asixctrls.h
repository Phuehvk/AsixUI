...
#error asixctrls.h requires asixapp.h to be included first
#endif

#ifndef __ATLWIN_H__
#error asixctrls.h requires atlwin.h to be included first
#endif
...
namespace Asix
{
	...
	public:
		...
		CTreeItemT<TBase> InsertItem(LPCTSTR lpszItem, int nImage, int nSelectedImage, HTREEITEM hParent, HTREEITEM hInsertAfter)
		{
			ATLASSERT(::IsWindow(m_hWnd));
			return InsertItem(TVIF_TEXT | TVIF_IMAGE | TVIF_SELECTEDIMAGE, lpszItem, nImage, nSelectedImage, 0, 0, 0, hParent, hInsertAfter); 
		}
		...
		CTreeItemT<TBase> InsertItem(UINT nMask, LPCTSTR lpszItem, int nImage, int nSelectedImage, UINT nState, UINT nStateMask, LPARAM lParam, HTREEITEM hParent, HTREEITEM hInsertAfter)
		{
			ATLASSERT(::IsWindow(m_hWnd));
			TVINSERTSTRUCT tvis = { 0 };
			tvis.hParent = hParent;
	...
	template <class TBase>
	inline BOOL CTreeItemT<TBase>::SetItem(UINT nMask, LPCTSTR lpszItem, int nImage, int nSelectedImage, UINT nState, UINT nStateMask, LPARAM lParam)
	{
		ATLASSERT(m_pTreeView != NULL);
	...
	public:
		...
		BOOL SetButtonInfo(int nID, DWORD dwMask, BYTE Style, BYTE State, LPCTSTR lpszItem, int iImage, WORD cx, int iCommand, DWORD_PTR lParam)
		{
			ATLASSERT(::IsWindow(m_hWnd));
			TBBUTTONINFO tbbi = { 0 };
	...
	#else
	#define MSFTEDIT_CLASS		"RICHEDIT50A"
	#endif // UNICODE
