...
#ifndef _ASIXRIBBON_INC_
#define _ASIXRIBBON_INC_
...
#ifndef _UNICODE
#error asixribbon.h requires the Unicode character set
#endif
#if !defined(NTDDI_WIN7) || (NTDDI_VERSION < NTDDI_WIN7)
#error asixribbon.h requires the _WIN32_WINNT at least 0x0601
#error asixribbon.h requires the Windows 7 SDK or higher
#endif


#ifndef _ASIXAPP_INC_
#error asixribbon.h requires asixapp.h to be included first
#endif


#include <asixmisc.h>    // for RecentDocumentList classes
#include <asixframe.h>   // for Frame and UpdateUI classes
#include <asixctrls.h>   // required for asixctrlw.h
#include <asixctrlw.h>   // for CCommandBarCtrl
...
namespace Asix
{
	...
	public:
		...
		{
			return bPersist ? UIAddElement<UPDUI_PERSIST>(nID) : UIRemoveElement<UPDUI_PERSIST>(nID);
		}
		...
		BOOL UISetText(int nID, UINT uIdResource, BOOL bForceUpdate = FALSE)
		{
			...
			return AtlLoadString(uIdResource, sText, RIBBONUI_MAX_TEXT) ? UISetText(nID, sText, bForceUpdate) : E_FAIL;
		}
	...
	namespace RibbonUI
	{
		...
		struct ICtrl
		{
			virtual HRESULT DoExecute(UINT nCmdID, UI_EXECUTIONVERB verb, const PROPERTYKEY* key, const PROPVARIANT* ppropvarValue, IUISimplePropertySet* pCommandExecutionProperties) = 0;

			virtual HRESULT DoUpdateProperty(UINT nCmdID, REFPROPERTYKEY key, const PROPVARIANT* ppropvarCurrentValue, PROPVARIANT* ppropvarNewValue) = 0;
		};
		...
		public:
			...
			virtual HRESULT DoExecute(UINT nCmdID, UI_EXECUTIONVERB verb, const PROPERTYKEY* key, const PROPVARIANT* ppropvarValue, IUISimplePropertySet* pCommandExecutionProperties)
			{
				ATLASSERT(nCmdID == t_ID);
				return GetWndRibbon().DoExecute(nCmdID, verb, key, ppropvarValue, pCommandExecutionProperties);
			}

			virtual HRESULT DoUpdateProperty(UINT nCmdID, REFPROPERTYKEY key, const PROPVARIANT* ppropvarCurrentValue, PROPVARIANT* ppropvarNewValue)
			{
				ATLASSERT(nCmdID == t_ID);
				...
				return (iVal <= iMax) && (iVal >= 0) ? OnGetText(key, ppropvarNewValue) : GetWndRibbon().DoUpdateProperty(nCmdID, key, ppropvarCurrentValue, ppropvarNewValue);
			}
		};
		...
		public:
			...
			HRESULT OnGetImage(REFPROPERTYKEY key, PROPVARIANT* ppv)
			{
				...
				return m_hbm[iImage].IsNull() ? E_NOTIMPL : SetPropertyVal(key, GetImage(m_hbm[iImage], UI_OWNERSHIP_COPY), ppv);
			}
			...
			virtual HRESULT DoUpdateProperty(UINT nCmdID, REFPROPERTYKEY key, const PROPVARIANT* ppropvarCurrentValue, PROPVARIANT* ppropvarNewValue)
			{
				...
				return (k_(key) <= k_SmallHighContrastImage) && (k_(key) >= k_LargeImage) ? OnGetImage(key, ppropvarNewValue) : CtrlImpl::DoUpdateProperty(nCmdID, key, ppropvarCurrentValue, ppropvarNewValue);
			}
		};
		...
		public:
			...
			HRESULT OnGetItem(UINT uIndex, REFPROPERTYKEY key, PROPVARIANT *value)
			{
				...
				return uIndex < t_items ? pCtrl->DoGetItem(uIndex, key, value) : pCtrl->DoGetCategory(uIndex - t_items, key, value);
			}
			...
			HRESULT DoUpdateProperty(UINT nCmdID, REFPROPERTYKEY key, const PROPVARIANT* ppropvarCurrentValue, PROPVARIANT* /*ppropvarNewValue*/)
			{
		...
		public:
			...
			HRESULT DoUpdateProperty(UINT nCmdID, REFPROPERTYKEY key, const PROPVARIANT* ppropvarCurrentValue, PROPVARIANT* ppropvarNewValue)
			{
				...
				if (k_(key) == k_SelectedItem)
				{
					...
					if ((m_uSelected == UI_COLLECTION_INVALIDINDEX) && ribbon.OnRibbonQuerySelectedItem(TCtrl::GetID(), uSel))
						m_uSelected = uSel;
		...
		public:
			...
			HRESULT DoGetItem(UINT uItem, REFPROPERTYKEY key, PROPVARIANT *value)
			{
				...
				if (k_(key) == k_ItemImage)
				{
					...
					return m_aBitmap[uItem].IsNull() ? E_NOTIMPL : SetPropertyVal(key, GetImage(m_aBitmap[uItem], UI_OWNERSHIP_COPY), value);
				}
				else
				{
		...
		public:
			...
			{
				TCtrl::WndRibbon& ribbon = static_cast<TCtrl*>(this)->GetWndRibbon();
				return ribbon.IsRibbonUI() ? ribbon.SetProperty(TCtrl::GetID(), UI_PKEY_StringValue, sText) : S_OK;
			}
		...
		class SimpleCollectionCtrlImpl : public CommandCtrlImpl<T, t_ID>, public SimpleCollectionImpl<SimpleCollectionCtrlImpl<T, t_ID, t_size, t_CommandType>, t_size, t_CommandType>
		{
		...
		class RecentItemsCtrlImpl : public CtrlImpl<T, t_ID>, public CollectionImplBase<RecentItemsCtrlImpl<T, t_ID, TDocList>, TDocList::m_nMaxEntries_Max>, public TDocList
		{
		...
		//
		template <class T>
		class CRibbonImpl : public CRibbonUpdateUI<T>, public ICtrl, public IUIApplication, public IUICommandHandler
		{
		...
		public:
			...
			UI_CONTROLDOCK GetQATDock()
			{
				...
				if ((pIPS != NULL) && SUCCEEDED(pIPS->GetValue(UI_PKEY_QuickAccessToolbarDock, &propvar)) && SUCCEEDED(UIPropertyToUInt32(UI_PKEY_QuickAccessToolbarDock, propvar, &uDock)))
			...
			UI_CONTEXTAVAILABILITY GetRibbonContextAvail(UINT32 uID)
			{
				...
				PROPVARIANT propvar;
				if (IsRibbonUI() && SUCCEEDED(GetIUIFrameworkPtr()->GetUICommandProperty(uID, UI_PKEY_ContextAvailable, &propvar)))
				{
					UINT uav;
			...
			HRESULT TrackRibbonMenu(UINT32 uID, INT32 x, INT32 y)
			{
				...
				return IsRibbonUI() ? ATL::CComPtr<IUIContextualUI>(GetMenuPtr(uID))->ShowAtLocation(x, y) : E_FAIL;
			}
			...
			UI_CONTEXTAVAILABILITY OnRibbonQueryTabAvail(UINT nCmdID)
			{
				...
				return ((dwState & UPDUI_DISABLED) == UPDUI_DISABLED) ? UI_CONTEXTAVAILABILITY_NOTAVAILABLE : (((dwState & UPDUI_CHECKED) == UPDUI_CHECKED) ? UI_CONTEXTAVAILABILITY_ACTIVE : UI_CONTEXTAVAILABILITY_AVAILABLE);
			}
			...
			{
				return IsRibbonUI() ? GetIUIFrameworkPtr()->InvalidateUICommand(nID, UI_INVALIDATIONS_ALLPROPERTIES, NULL) : E_FAIL;
			}
			...
			// IUICommandHandler
			STDMETHODIMP Execute(UINT nCmdID, UI_EXECUTIONVERB verb, const PROPERTYKEY* key, const PROPVARIANT* ppropvarValue, IUISimplePropertySet* pCommandExecutionProperties)
			{
				T* pT =static_cast<T*>(this);
				return pT->GetRibbonCtrl(nCmdID).DoExecute(nCmdID, verb, key, ppropvarValue, pCommandExecutionProperties);	
			}

			STDMETHODIMP UpdateProperty(UINT nCmdID, REFPROPERTYKEY key, const PROPVARIANT* ppropvarCurrentValue, PROPVARIANT* ppropvarNewValue)
			{
				T* pT =static_cast<T*>(this);
			...
			STDMETHODIMP QueryInterface(REFIID iid, void** ppv)
			{
				if ((iid == __uuidof(IUnknown)) || (iid == __uuidof(IUICommandHandler)) || (iid == __uuidof(IUIApplication)))
				{
					*ppv = this;
					return S_OK;
				}
	...
	*/
	...
	public:
		// Construction
		CRibbonFrameWindowImplBase(bool bUseCommandBarBitmaps = true) : m_bUseCommandBarBitmaps(bUseCommandBarBitmaps), m_bWin7Fix(false)
		{
