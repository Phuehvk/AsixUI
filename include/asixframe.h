...
#ifndef _ASIXFRAME_INC_
#define _ASIXFRAME_INC_
...
#error asixframe.h requires asixapp.h to be included first
#endif

#ifndef __ATLWIN_H__
#error asixframe.h requires atlwin.h to be included first
#endif
...
namespace Asix
{
	...
	static Asix::CFrameWndClassInfo& GetWndClassInfo() \
	{ \
		static Asix::CFrameWndClassInfo wc = \
		{ \
			{ sizeof(WNDCLASSEX), 0, StartWindowProc, 0, 0, NULL, NULL, NULL, (HBRUSH)(COLOR_WINDOW + 1), NULL, WndClassName, NULL }, \
	...
	{ \
		static Asix::CFrameWndClassInfo wc = \
		{ \
			{ sizeof(WNDCLASSEX), style, StartWindowProc, 0, 0, NULL, NULL, NULL, (HBRUSH)(bkgnd + 1), NULL, WndClassName, NULL }, \
			NULL, NULL, IDC_ARROW, TRUE, 0, _T(""), uCommonResourceID \
		}; \
		return wc; \
	}\
	...
	{ \
		static Asix::CFrameWndClassInfo wc = \
		{ \
			{ sizeof(WNDCLASSEX), 0, StartWindowProc, 0, 0, NULL, NULL, NULL, NULL, NULL, WndClassName, NULL }, \
	...
	public:
		...
		HWND Create(HWND hWndParent, ATL::_U_RECT rect, LPCTSTR szWindowName, DWORD dwStyle, DWORD dwExStyle, ATL::_U_MENUorID MenuOrID, ATOM atom, LPVOID lpCreateParam)
		{
			...
			HWND hWnd = ::CreateWindowEx(dwExStyle, MAKEINTATOM(atom), szWindowName, dwStyle, rect.m_lpRect->left, rect.m_lpRect->top, rect.m_lpRect->right - rect.m_lpRect->left, rect.m_lpRect->bottom - rect.m_lpRect->top, hWndParent, MenuOrID.m_hMenu, ModuleHelper::GetModuleInstance(), lpCreateParam);
		...
		static HWND CreateSimpleToolBarCtrl(HWND hWndParent, UINT nResourceID, BOOL bInitialSeparator = FALSE, DWORD dwStyle = ATL_SIMPLE_TOOLBAR_STYLE, UINT nID = ATL_IDW_TOOLBAR)
		{
			...
			CTempBuffer<TBBUTTON, _ASIX_STACK_ALLOC_THRESHOLD> buff;
	...
	inline HWND AtlCreateSimpleToolBar(HWND hWndParent, UINT nResourceID, BOOL bInitialSeparator = FALSE, DWORD dwStyle = ATL_SIMPLE_TOOLBAR_STYLE, UINT nID = ATL_IDW_TOOLBAR)
	{
	...
	#ifndef _ASIX_MDIWINDOWMENU_TEXT
	#define _ASIX_MDIWINDOWMENU_TEXT	_T("&Window")
	#endif
	...
	public:
		...
		static HMENU GetStandardWindowMenu(HMENU hMenu)
		{
			...
			if(::GetMenuString(hMenu, nCount - 2, lpszText, nLen + 1, MF_BYPOSITION) != nLen)
				return NULL;
			if(lstrcmp(lpszText, _ASIX_MDIWINDOWMENU_TEXT) != 0)
				return NULL;
	...
	public:
		...
		HWND CreateMDIClient(HMENU hWindowMenu = NULL, UINT nID = ATL_IDW_CLIENT, UINT nFirstChildID = ATL_IDM_FIRST_MDICHILD)
		{
			...
			m_hWndClient = ::CreateWindowEx(dwExStyle, _T("MDIClient"), NULL, dwStyle, 0, 0, 1, 1, m_hWnd, (HMENU)LongToHandle(nID), ModuleHelper::GetModuleInstance(), (LPVOID)&ccs);
			if (m_hWndClient == NULL)
			{
	...
	public:
		...
		BOOL UpdateClientEdge(LPRECT lpRect = NULL)
		{
			...
			if(dwStyle != dwNewStyle)
			{
				...
				::RedrawWindow(m_hWndMDIClient, NULL, NULL, RDW_INVALIDATE | RDW_ALLCHILDREN);
	...
	public:
		...
		BOOL UIUpdateStatusBar(BOOL bForceUpdate = FALSE)
		{
			...
			for(int i = 0; i < m_arrUIMap.GetSize(); i++)
			{
				...
				{
					if((m_UIElements[e].m_wType == UPDUI_STATUSBAR) && (m_arrUIMap[i].m_wType & UPDUI_STATUSBAR) && (m_arrUIData[i].m_wState & UPDUI_STATUSBAR))
					{
