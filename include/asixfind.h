...
#ifndef _ASIXFIND_INC_
#define _ASIXFIND_INC_

#pragma once


#ifndef _ASIX_USE_CSTRING
#include <atlstr.h>
#else
#ifndef _ASIXMISC_INC_
#include <asixmisc.h>
#endif
#endif
...
#ifndef _ASIXCTRLS_INC_
#error asixfind.h requires asixctrls.h to be included first
#endif

#ifndef _ASIXDLGS_INC_
#error asixfind.h requires asixdlgs.h to be included first
#endif

#if !((defined(_ASIXMISC_INC_) && defined(_ASIX_USE_CSTRING)) || defined(__ATLSTR_H__))
#error asixfind.h requires CString (either from ATL's atlstr.h or Asix's asixmisc.h with _ASIX_USE_CSTRING)
#endif
...
namespace Asix
{
	...
	public:
		// Constructors
		CEditFindReplaceImplBase() : m_pFindReplaceDialog(NULL), m_bFindOnly(TRUE), m_bFirstSearch(TRUE), m_bMatchCase(FALSE), m_bWholeWord(FALSE), m_bFindDown(TRUE), m_nInitialSearchPos(0), m_hOldCursor(NULL)
		{
		}
		...
		LRESULT OnFindReplaceCmd(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM lParam, BOOL& /*bHandled*/)
		{
			...
			if((m_pFindReplaceDialog != NULL) && (findReplace != NULL))
			{
				if(pDialog->FindNext())
				{
					pT->OnFindNext(pDialog->GetFindString(), pDialog->SearchDown(), pDialog->MatchCase(), pDialog->MatchWholeWord());
				}
				else if(pDialog->ReplaceCurrent())
				{
					pT->OnReplaceSel(pDialog->GetFindString(), pDialog->SearchDown(), pDialog->MatchCase(), pDialog->MatchWholeWord(), pDialog->GetReplaceString());
				}
				else if(pDialog->ReplaceAll())
				{
					pT->OnReplaceAll(pDialog->GetFindString(), pDialog->GetReplaceString(), pDialog->MatchCase(), pDialog->MatchWholeWord());
				}
		...
		// bFindOnly: TRUE for Find, FALSE for FindReplace
		TFindReplaceDialog* CreateFindReplaceDialog(BOOL bFindOnly, LPCTSTR lpszFindWhat, LPCTSTR lpszReplaceWith = NULL, DWORD dwFlags = FR_DOWN, HWND hWndParent = NULL)
		{
			...
			{
				HWND hWndFindReplace = findReplaceDialog->Create(bFindOnly, lpszFindWhat, lpszReplaceWith, dwFlags, hWndParent);
		...
		void FindReplace(BOOL bFindOnly)
		{
			...
			m_pFindReplaceDialog = pT->CreateFindReplaceDialog(bFindOnly, findNext, replaceWith, dwFlags, pT->operator HWND());
			ATLASSERT(m_pFindReplaceDialog != NULL);
			if(m_pFindReplaceDialog != NULL)
		...
		BOOL SameAsSelected(LPCTSTR lpszCompare, BOOL bMatchCase, BOOL /*bWholeWord*/)
		{
			...
			return (bMatchCase && selectedText.Compare(lpszCompare) == 0) || (!bMatchCase && selectedText.CompareNoCase(lpszCompare) == 0);
		}
		...
		void OnReplaceAllCoreEnd(int replaceCount)
		{
			...
			if(message.GetLength() > 0)
			{
				...
				if(m_pFindReplaceDialog != NULL)
				{
					m_pFindReplaceDialog->MessageBox(formattedMessage, pT->GetTranslationText(eText_OnReplaceAllTitle), MB_OK | MB_ICONINFORMATION | MB_APPLMODAL);
				}
				else
				{
					pT->MessageBox(formattedMessage, pT->GetTranslationText(eText_OnReplaceAllTitle), MB_OK | MB_ICONINFORMATION | MB_APPLMODAL);
				}
			}
		}
		...
		void OnTextNotFound(LPCTSTR lpszFind)
		{
			...
			if(message.GetLength() > 0)
			{
				...
				if(m_pFindReplaceDialog != NULL)
				{
					m_pFindReplaceDialog->MessageBox(formattedMessage, pT->GetTranslationText(eText_OnTextNotFoundTitle), MB_OK | MB_ICONINFORMATION | MB_APPLMODAL);
				}
				else
				{
					pT->MessageBox(formattedMessage, pT->GetTranslationText(eText_OnTextNotFoundTitle), MB_OK | MB_ICONINFORMATION | MB_APPLMODAL);
				}
			}
			else
			{
	...
	public:
		// Constructors
		CEditFindReplaceImpl() : m_pShadowBuffer(NULL), m_nShadowSize(0), m_bShadowBufferNeeded(-1)
		{
		}
