...
#ifndef _ASIXCTRLW_INC_
#define _ASIXCTRLW_INC_
...
#error asixctrlw.h requires asixapp.h to be included first
#endif

#ifndef _ASIXCTRLS_INC_
#error asixctrlw.h requires asixctrls.h to be included first
#endif

// Define _ASIX_CMDBAR_VISTA_MENUS as 0 to exclude Vista menus support
#if !defined(_ASIX_CMDBAR_VISTA_MENUS)
#define _ASIX_CMDBAR_VISTA_MENUS 1
#endif
...
namespace Asix
{
	...
	public:
		...
		BEGIN_MSG_MAP(CCommandBarCtrlImpl)
			...
			// public API handlers - these stay to support chevrons in asixframe.h
		...
		{
			bHandled = FALSE;
			if((m_uSysKey == VK_MENU || (m_uSysKey == VK_F10 && !(::GetKeyState(VK_SHIFT) & 0x80)) || m_uSysKey == VK_SPACE) && wParam == SC_KEYMENU)
			{
				T* pT = static_cast<T*>(this);
				if(::GetFocus() == m_hWnd)
				{
		...
		void DoPopupMenu(int nIndex, bool bAnimate)
		{
			...
			SetHotItem(nCmdID);
			pT->DoTrackPopupMenu(hMenuPopup, TPM_LEFTBUTTON | TPM_VERTICAL | TPM_LEFTALIGN | TPM_TOPALIGN | (s_bW2K ? (bAnimate ? TPM_VERPOSANIMATION : TPM_NOANIMATION) : 0), pt.x, pt.y, &TPMParams);
		...
		static UINT GetAutoPopupMessage()
		{
			...
			if(uAutoPopupMessage == 0)
			{
				...
				if(uAutoPopupMessage == 0)
					uAutoPopupMessage = ::RegisterWindowMessage(_T("AsixUI_CmdBar_InternalAutoPopupMsg"));
		...
		static UINT GetGetBarMessage()
		{
			...
			if(uGetBarMessage == 0)
			{
				...
				if(uGetBarMessage == 0)
					uGetBarMessage = ::RegisterWindowMessage(_T("AsixUI_CmdBar_InternalGetBarMsg"));
	...
	{
	public:
		DECLARE_WND_SUPERCLASS(_T("AsixUI_CommandBar"), GetWndClassName())
	};
	...
	public:
		...
		LRESULT OnNcLButtonDown(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM lParam, BOOL& bHandled)
		{
			...
			if(::PtInRect(&rcIcon, pt))
			{
				...
				UINT uRet = (UINT)menu.TrackPopupMenu(TPM_LEFTBUTTON | TPM_VERTICAL | TPM_LEFTALIGN | TPM_TOPALIGN | TPM_RETURNCMD | (s_bW2K ? TPM_VERPOSANIMATION : 0), m_bLayoutRTL ? rect.right : rect.left, rect.bottom, m_hWndChildMaximized);
	...
	{
	public:
		DECLARE_WND_SUPERCLASS(_T("AsixUI_MDICommandBar"), GetWndClassName())
	};

}	// namespace Asix

#endif // _ASIXCTRLW_INC_
