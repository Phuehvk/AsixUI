...
#ifndef _ASIXDDX_INC_
#define _ASIXDDX_INC_

#pragma once

#ifndef _ASIXAPP_INC_
	#error asixddx.h requires asixapp.h to be included first
#endif
...
// Note: Specialized versions require asixctrls.h to be included first
#if (_MSC_VER >= 1300)
...
#ifdef _ASIXCTRLS_INC_
  #define DDX_TAB_INDEX(nID, var)      DDX_INDEX(Asix::CTabCtrl, nID, var)
  #define DDX_COMBO_INDEX(nID, var)    DDX_INDEX(Asix::CComboBox, nID, var)
  #define DDX_LISTBOX_INDEX(nID, var)  DDX_INDEX(Asix::CListBox, nID, var)
  #define DDX_LISTVIEW_INDEX(nID, var) DDX_INDEX(Asix::CListViewCtrl, nID, var)
...
// DDX support for Tab, Combo, ListBox and ListView selection index
	...
	template <>
	INT _getSel(Asix::CListViewCtrl& tCtrl)
	{
		return tCtrl.GetSelectedIndex();
	}

	template <>
	void _setSel(Asix::CListViewCtrl& tCtrl, INT iSel)
	{
		if(iSel < 0)
			tCtrl.SelectItem(-1);
		else
